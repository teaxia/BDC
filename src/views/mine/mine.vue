...
export default {
	...
	data() {
		...
		// @2.10.40 茶树虾	2019-05-28	所有安全码更改为弹出层方式输入;取消充值、办理信用卡、取消首页游戏钱包、取消资产转换;移动OTC至导航;增加在线兑换资产选择、提币增加提币类型选;在线兑换增加确认提示、提币默认选择USD;增加谷歌认证;
		// @2.11.41 茶树虾	2019-06-04  修复部分输入安全码没有按照弹除层的形式输入；锁仓时间描述更改；增加BDPay功能（与网站充值结合）
		// @2.12.42 茶树虾	2019-06-04  绑定银行卡新增删除功能、调整OTC界面单价更改为发布价格、调整OTC发布售币最低限额为CNY单位并且不能低于100、增加OTC订单界面可申诉倒计时
		// @2.12.43 茶树虾	2019-06-04  OTC改为接口判断权限、发送类别的取消固定资产发送
		// @2.13.44 茶树虾	2019-07-01  开放OTC接口
		// @2.13.45 茶树虾	2019-07-02  优化部分界面、OTC待发币状态可申诉、更改部分文字描述
		// @2.14.46 茶树虾	2019-07-02  优化部分界面、增加场外买币可以强制发币功能
		// @2.15.48 茶树虾	2019-07-08  修复发布编辑界面为撤销（-3）状态时，不停调用接口的BUG；修改部分样式
		// @2.16.49 茶树虾	2019-07-11  求购单，卖家，待支付状态，给倒计时提示“等待买家付款，剩余xx分xx秒超时自动取消”；发布求购接口揉和、发布范围取消全部，增加从接口判断；求购只有场内；修复支付二维码下载某些安卓机型无法保存图片的问题，增加保存图片不成功的提示
		// @2.16.50 茶树虾	2019-07-15  优化排行榜样式；增加信誉度显示；买卖页面增加发布时间显示
		// @2.17.51 茶树虾	2019-07-16  修复绑定支付信息过多时，无法通过下拉选择的BUG；修改OTC收益查询时间范围；修改账本样式
		// @2.17.52 茶树虾	2019-07-23  调整OTC收益界面，通过接口调用日期数据，尝试修复上传图片的jm错误
		// @2.18.53 茶树虾	2019-07-26  优化上传图片；绑定收款方式增加地区选择；优化OTC界面信息；修复选择按钮的穿透问题
		// @2.19.54 茶树虾	2019-07-31  增加OTC报表功能，重新开放资产转换（通证转固定）
		// @2.20.55 茶树虾	2019-07-31  修改矿区展示、支付宝微信绑定显示绑定城市、常见问题修改为OTC操作指南、新增币种转换
		// @2.21.55 茶树虾	2019-08-05  更新新强制发币的银行卡选项（我的直推隐藏）
	},
	methods: {
		// 退出
		logout(){
